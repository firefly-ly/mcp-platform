...
const PROMPT_INJECTION_RULES = [
  ...
  { id: "INJ-005", category: "隐匿编码", severity: "alert",
    // eslint-disable-next-line no-misleading-character-class -- 零宽字符本就是检测目标（U+200C/200D 为合法连接符，非误报）
    re: /[\u200b\u200c\u200d\ufeff]/ },
...
async function runTrivyScan(id) {
  ...
  try {
    ...
    let json;
    try { json = JSON.parse(out.stdout); } catch (e) { throw new Error("trivy 输出解析失败", { cause: e }); }
