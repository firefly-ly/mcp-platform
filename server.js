...
// 实例缓存失效钩子：routes/mcp 装配时注入实现（双清 toolsCache/rtCache）
let mcpRuntimeInvalidate = null;
function invalidateMcpRuntimeCaches(id) {
  if (typeof mcpRuntimeInvalidate === "function") mcpRuntimeInvalidate(id);
}

async function deployMcp(id) {
  invalidateMcpRuntimeCaches(id); // 部署/停止双清 tools/rt 两层缓存（routes/mcp 注入）
  const sub = db.prepare("SELECT * FROM submissions WHERE id=?").get(id);
...
mcpRuntimeInvalidate = require("./routes/mcp")(app, {
  ...CTX,
...
require("./routes/submissions")(app, {
  ...
  deniedThrottled, runSubmissionScans, deleteSubmissionSecrets, storeSubmissionEnv, undeployMcp,
