...
async function openSkillPackage(buffer, lowerKey) {
  ...
  if (list.err) { try { fs.unlinkSync(tmp); } catch (_) {} return { error: "包内列表读取失败: " + (list.stderr || list.err.message) }; }
  return {
